...
class TorchSTFT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # complex_stft = torch.Size([1, 2049, 18]) sample, f, t 所有元素都是複數0.+0.j...
        # magnitude = torch.abs(complex_stft)
        # phase = torch.angle(complex_stft)
        # print(magnitude.shape, '123')
        # print(phase.shape)
...
if __name__ == '__main__':
    data = torch.zeros((1, 1, 22400))
    STFT = TorchSTFT(n_fft=4096, n_hop=1024)
    com = ComplexNorm()
    print(STFT(data).shape)
    print(com(STFT(data)).shape)